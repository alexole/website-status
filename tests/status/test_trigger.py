...
from src.status.trigger import trigger_website_checks

# Mark all tests as coroutines.
pytestmark = pytest.mark.asyncio


@patch('src.status.trigger.AIOKafkaProducer')
async def test_trigger_website_checks(mock_kafka_producer, website):
    ...
    '''
    with patch('src.status.trigger.open', mock_open(read_data=json_data.encode())):
